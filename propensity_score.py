...
def estimate_ps(df_cov: pd.DataFrame, y: np.ndarray,
    ...
    cat: list of str, default=None
        Categorical covariates in `df_cov`. If None, assumes no categorical
...
def stratify_ps(df_match: pd.DataFrame, n_strata: int = 10):
    """ Stratify subjects based on their estimated propensity scores.
    ...
    df_stratified: pd.DataFrame
        Input FataFrame with an additional column 'strata' indicating the
        stratum of each subject.
    """
    ps_mean = df_match.groupby('pair').mean().ps
    strata = pd.qcut(ps_mean, q=n_strata, labels=False)
    df_match['strata'] = df_match.apply(lambda x: strata[x['pair']], axis=1)
    return df_match
